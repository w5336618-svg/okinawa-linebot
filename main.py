...
def search_reels(query: str, top_n: int = 15) -> str:
    keywords = [w for w in query if w.strip()]
    query_words = list(query.replace('，', ' ').replace('？', ' ').replace('?', ' ').split())
    scored = []
    for r in ALL_REELS:
        caption = r.get('caption', '')
        score = sum(1 for w in query_words if w and w in caption)
        if score > 0:
            scored.append((score, r))
    scored.sort(key=lambda x: -x[0])
    top = scored[:top_n] if scored else ALL_REELS[:top_n]
    lines = []
    for item in top:
        r = item[1] if isinstance(item, tuple) else item
        lines.append(f"- {r['caption']} → https://www.instagram.com/reel/{r['code']}/")
    return '\n'.join(lines)
...
def ask_claude(user_message: str) -> str:
    ...
    relevant = search_reels(user_message)
    examples = build_examples_prompt()
    system = BASE_SYSTEM_PROMPT
    if relevant:
        system += f'\n\n以下是與這個問題最相關的影片（最多15支）：\n{relevant}'
    if examples:
        system += f'\n\n{examples}'
